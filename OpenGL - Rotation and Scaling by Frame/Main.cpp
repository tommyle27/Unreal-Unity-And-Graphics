...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) // keep window open until it is closed
	{
		...
		transform = glm::rotate(transform, (float)glfwGetTime(), glm::vec3(0.0f, 0.0f, 1.0f)); // rotate every frame
		...
		// Draw second square make it scale every frame
		...
		ourShader.use();
		unsigned int transformLocTwo = glGetUniformLocation(ourShader.ID, "transform");
		glUniformMatrix4fv(transformLocTwo, 1, GL_FALSE, glm::value_ptr(transform));
