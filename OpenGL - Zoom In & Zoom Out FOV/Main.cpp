...
void changeFOV(GLFWwindow* window);
...
int main()
{
	...
	while (!glfwWindowShouldClose(window)) // keep window open until it is closed
	{
		...
		// render 
		glClearColor(0.2f, 0.6f, 0.3f, 1.0f); // clear the colour of the buffer and give it another colour
...
void changeFOV(GLFWwindow* window) {
	if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
		FOV += 1.0f;
		if (FOV >= 150.0f) FOV = 120.0f;
	}

	if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) {
		FOV -= 1.0f;
		if (FOV <= 0.0f) FOV = 10.0f;
	}
}